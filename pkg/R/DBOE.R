      ...
      get.metadata = function(..., chatty = FALSE){
        ...
        invisible(self);
			...
			make.virtual_database = function(conn, ..., target_env = rlang::caller_env(), sch = "dbo"){
				...
				if (!missing(sch)){ sch <- rlang::enexpr(sch) |> as.character() }
				...
				.tables <- if (!rlang::is_empty(.tables)){
						...
						if (!rlang::is_empty(.out)){
							unique(.out) |>
								data.table::setnames(
									c("table_schema", "view_name")
									, c("schema_name", "tbl_name")
									, skip_absent = TRUE
									)
						} else { .out }
					}
				...
				list(.tables, .views) |>
					...
					(\(x){
						# Added schema selection support 2023.08.03 | https://github.com/delriaan/DBOE/issues/6
						if (!rlang::is_empty(sch)){ x[(schema_name == sch)] } else { x }
					})() |>
